...
def make_spatial_cloudlet_connections(cloudlets, MC):
    ...
    cloudlets are bordering other cloudlets.
    """
...
def advect_indexes(indexes, u, v, w, MC):
    # Units for u, v, w are grid cells/time step
...
def make_temporal_connections(cloudlets, old_clusters, MC):
    """For each cloudlet, find the previous time step's cluster that overlaps the 
    cloudlet the most.
    """
    ...
    # Label the cloud condensed and plume points using the list index of the
    # cluster
    ...
    for cloudlet in cloudlets:
        # Find condensed-condensed overlaps
        if cloudlet.has_condensed():
            # Correct for cloud advection
            advected_condensed_mask = advect_indexes(
                cloudlet.condensed_mask(), cloudlet.u['condensed'],
                cloudlet.v['condensed'], cloudlet.w['condensed'], MC)

            # Get indexes of previous condensed region from the advected array
            ...
            # Find condensed-plume overlaps (cloudlet: condensed, cluster: plume)
        ...
        # Find plume-condensed overlaps (cloudlet: plume, cluster: condensed)
        advected_plume_mask = advect_indexes(
            cloudlet.plume_mask(), cloudlet.u['plume'], cloudlet.v['plume'],
            cloudlet.w['plume'], MC)
...
def associate_cloudlets_with_previous_clusters(cloudlets, old_clusters, MC):
    ...
    for cloudlet in cloudlets:
        ...
        # If there are back connections, add the cloudlet to a cluster
        if max_conn > -1:
...
def check_for_adjacent_cloudlets(new_cloudlets, clusters):
    """Check the clusters list to verify if any of the cloudlets which did not
...
def make_clusters(cloudlets, old_clusters, MC):
    ...
    # be associated are assumed to be newly created.
    new_cloudlets, current_clusters = associate_cloudlets_with_previous_clusters(
        cloudlets, old_clusters, MC)

    # Verify if any of the new cloudlets are touching a cluster
    check_for_adjacent_cloudlets(new_cloudlets, current_clusters)

    # Verify if the cloudlets in a cluster are no longer touching
    max_id = split_clusters(current_clusters, max_id, MC)

    # Create new clusters from any leftover new cloudlets
    final_clusters = create_new_clusters(
        new_cloudlets, current_clusters, max_id, MC)
